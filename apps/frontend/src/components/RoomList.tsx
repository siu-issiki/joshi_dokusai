 ...
 */
export default function RoomList({ onJoinRoom, onCreateRoom, loading: externalLoading = false }: RoomListProps) {
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600">
            ...
            {!connected && <span className="text-red-600 ml-2">（接続中...）</span>}
          </p>
        </div>
        <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => setMaxPlayersFilter(e.target.value ? Number(e.target.value) : null)}
              className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
            >
          ...
          <div className="flex items-center">
            <label className="flex items-center">
              <input
                ...
                className="mr-2"
              />
              <span className="text-sm text-gray-700">プライベートルームを表示</span>
            </label>
          </div>
        ...
        {filteredRooms.length === 0 ? (
          <div className="text-center py-12 bg-white rounded-lg shadow-md">
            <div className="text-gray-500">
              <div className="text-lg font-semibold mb-2">利用可能なルームがありません</div>
              <div className="text-sm">新しいルームを作成してゲームを始めましょう！</div>
            </div>
          </div>
        ) : (
          filteredRooms.map((room) => (
            <div key={room.id} className="bg-white rounded-lg shadow-md p-4 hover:shadow-lg transition-shadow">
              <div className="flex justify-between items-center">
                <div className="flex-1">
                  ...
                  <div className="flex items-center space-x-4 mt-2 text-sm text-gray-600">
                    ...
                    <span>
                      👑 {Object.values(room.players).find(p => p.id === room.createdBy)?.name || '不明'}
                    </span>
                  </div>
                </div>
