...
export const startGame = onCall(async (request) => {
  ...
  try {
    ...
    if (playerCount < GAME_CONFIG.MIN_PLAYERS || playerCount > GAME_CONFIG.MAX_PLAYERS) {
      throw new Error(`プレイヤー数が不正です（${GAME_CONFIG.MIN_PLAYERS}-${GAME_CONFIG.MAX_PLAYERS}人必要）`);
    }
    ...
    const gamePlayersData: Record<string, FirebaseGamePlayer> = {};

    // Store the sorted player order for consistent indexing
    const playerOrder = players.map((player: FirebaseRoomPlayer) => String(player.id));

    players.forEach((player: FirebaseRoomPlayer, index: number) => {
      ...
      gamePlayersData[player.id] = {
        ...
        role: role,
        life: role === 'boss' ? GAME_CONFIG.BOSS_INITIAL_LIFE : GAME_CONFIG.SUBORDINATE_INITIAL_LIFE,
        maxLife: role === 'boss' ? GAME_CONFIG.BOSS_INITIAL_LIFE : GAME_CONFIG.SUBORDINATE_INITIAL_LIFE,
    ...
    const handPromises = players.map(async (player: FirebaseRoomPlayer, index: number) => {
      const role = index === 0 ? 'boss' : 'subordinate';
      const handSize = role === 'boss' ? GAME_CONFIG.BOSS_INITIAL_HAND_SIZE : GAME_CONFIG.SUBORDINATE_INITIAL_HAND_SIZE;
    ...
    await db.ref(`games/${gameId}/gameState/deckCount`).set(currentDeck.workCards.length);
    // workCardsデッキ自体も保存（カードドロー用）
    await db.ref(`games/${gameId}/gameState/workCardsDeck`).set(currentDeck.workCards);
