...
export const playCard = onCall(async (request) => {
  ...
  try {
    ...
    if (Object.keys(effectResult.playerUpdates).length > 0) {
      const playerUpdatePromises = Object.entries(effectResult.playerUpdates).map(([playerId, updates]) => {
        return gameRef.child(`players/${playerId}`).update(updates);
      });
      await Promise.all(playerUpdatePromises);
    }

    // ゲーム状態を更新
    if (effectResult.gameStateUpdates && Object.keys(effectResult.gameStateUpdates).length > 0) {
