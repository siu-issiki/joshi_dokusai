...
export const nullifyDictatorshipCard = onCall(async (request) => {
  ...
  try {
    ...
    const nullificationsUsed = game.gameState.dictatorshipEffects.nullificationsUsed;
    const maxNullifications = playerCount === 5 ? GAME_CONFIG.NULLIFICATION_LIMIT_4_PLAYERS : GAME_CONFIG.NULLIFICATION_LIMIT_3_PLAYERS;

    const currentNullifications = playerCount === 5 ? nullificationsUsed.boss4Players : nullificationsUsed.boss3Players;
    ...
    // 無効化回数を増加
    if (playerCount === 5) {
      updates['gameState/dictatorshipEffects/nullificationsUsed/boss4Players'] = currentNullifications + 1;
    } else {
      updates['gameState/dictatorshipEffects/nullificationsUsed/boss3Players'] = currentNullifications + 1;
    }
